...
//                new UserMeal(LocalDateTime.of(2020, Month.JANUARY, 30, 11, 0), "Второй завтрак", 250)
        );
...
//        System.out.println(filteredByStreams(meals, LocalTime.of(7, 0), LocalTime.of(12, 0), 2000));
    ...
    public static List<UserMealWithExcess> filteredByCycles(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> datesSummary = new HashMap<>();
        for (UserMeal meal : meals) {
            LocalDate date = meal.getDateTime().toLocalDate();
            datesSummary.merge(date, meal.getCalories(), Integer::sum);
        }
        ...
        for (UserMeal meal : meals) {
            if (meal.getDateTime().toLocalTime().isAfter(startTime) &&
                ...
                UserMealWithExcess mealWithExcess = new UserMealWithExcess(meal.getDateTime(),
                        ...
                        datesSummary.get(meal.getDateTime().toLocalDate()) > caloriesPerDay);
    ...
    public static List<UserMealWithExcess> filteredByStreams(List<UserMeal> meals, LocalTime startTime, LocalTime endTime, int caloriesPerDay) {
        Map<LocalDate, Integer> caloriesCounted = meals.stream()
        ...
        return meals.stream()
                ...
                .map(meal -> new UserMealWithExcess(meal.getDateTime(), meal.getDescription(),
                        meal.getCalories(), caloriesCounted.get(meal.getDateTime().toLocalDate()) > caloriesPerDay))
